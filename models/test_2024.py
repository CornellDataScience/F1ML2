"""
Test Models on 2024 Data

This script tests our best models on the 2024 season to see how well
...
print("=" * 70)
print("TESTING MODELS ON 2024 DATA")
print("Using LEAK-FREE datasets")
print("=" * 70)

# Load leak-free training dataset
print("\nLoading leak-free training dataset (1983-2024)...")
train_df = pd.read_csv('../data/HOLY_qualifying_v1_train2025.csv')
train_df = train_df[train_df['season'] < TEST_YEAR].copy()
...
df = pd.read_csv('../data/HOLY_qualifying_v1_train2025.csv')
...
# Split data: Train on <2024, Test on 2024
print("\n" + "=" * 70)
print("DATA SPLIT")
print("=" * 70)
...
# Filter out rows with missing grid positions (incomplete qualifying data)
missing_grid = test_df['grid'].isna().sum()
if missing_grid > 0:
    print(f"⚠️  Removing {missing_grid} records with missing qualifying results")
    missing_rounds = test_df[test_df['grid'].isna()]['round'].unique()
    print(f"   Missing rounds: {sorted(missing_rounds)}")
    test_df = test_df[test_df['grid'].notna()].copy()

print(f"Training set: {len(train_df)} records (1983-2023)")
...
# ============================================================================
print("\n" + "=" * 70)
print("CORRECTLY PREDICTED POLES IN 2024")
print("=" * 70)
...
# Check which model is better for 2024
print("\n" + "=" * 70)
print("WINNER FOR 2024")
print("=" * 70)
...
avg_2024 = (pole_acc_br + pole_acc_lr) / 2
avg_mae = (mae_br + mae_lr) / 2

print(f"Pole Position Accuracy:")
print(f"  Average 2023 performance: {avg_2024:.2f}%")
